...
class Game(cmd.Cmd):
    ...
    def start(self):
        self.terminal.start() # start terminal then start cmd reading loop
        self.cmdloop()
    ...
    def do_test(self, arg):
        self.terminal.choice_modal("Test","Testing!", ["1", "2"])
    ...
    def default(self, line):
        self.terminal.output("Command not found. Please try again!")
    ...
    def cmdloop(self, intro=None):
        ...
        try:
            ...
            stop = False
            for line in self.terminal.read_cmd():
                ...
                status = [
                    ...
                    "-----------",
                    self.current_cave.description,
                    f"This cave is linked to caves {linked_caves_str}"
                ]
                ...
                if len(characters) > 0:
                    status.append("You aren't alone in here. You see:")
                    for c in characters:
                    ...
                    status.append("It seems like there is no one else here")
